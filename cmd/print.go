...
func (pr *PrintRunner) PrintResources(cmd *cobra.Command, _ []string) error {
	...
	pr.outputResult(gatewayResources, additionalResources...)
	return nil
}

func (pr *PrintRunner) outputResult(resources i2gw.GatewayResources, additionalResources ...*unstructured.Unstructured) {
	...
	for _, r := range resources.TLSRoutes {
		err := pr.resourcePrinter.PrintObj(&r, os.Stdout)
		if err != nil {
			fmt.Printf("# Error printing %s/%s TLSRoute: %v\n", r.Namespace, r.Name, err)
		}
	}
	...
	for _, r := range resources.TCPRoutes {
		err := pr.resourcePrinter.PrintObj(&r, os.Stdout)
		if err != nil {
			fmt.Printf("# Error printing %s/%s TCPRoute: %v\n", r.Namespace, r.Name, err)
		}
	}
